...
public class BTree {
	...
	private BTree rakiBaseColumnFiletree;
	...
	public BTree(RandomAccessFile file, String tableName) {
		...
		try {
			...
			if (!tableName.equals("rakibase_columns")
					&& !tableName.equals("rakibase_tables")) {
				rakiBaseColumnFiletree = new BTree(new RandomAccessFile(
						RakiDB.rakibase_columns, "rw"), "rakibase_columns",
						true, false);

				for (String key : rakiBaseColumnFiletree.getSchema(tableName)
						.keySet()) {
					tableKey = key;
	...
	public BTree(RandomAccessFile file, String tableName, boolean isColSchema,
		...
		this(file, tableName);
		isColumnSchema = isColSchema;
		this.isTableSchema = isTableSchema;

	}

	public void createNewInterior(int pageNumber, int rowID, int pageRight) // Rakesh
	{
		try {
			binaryFile.seek(0);
			binaryFile.write(5);
			...
			try {
				...
				if (actualRowID == rowId) {
					...
					if (isColumnSchema) {
						...
						token = rakiBaseColumnFiletree.getSchema(tableName);
					}
	...
	private void writeCell(int pageLocation,
		...
		writeCellHeader(pageLocation, true,
				Integer.parseInt(token.get(tableKey).get(1)), no_of_Bytes);
	...
	private void writeCellHeader(int pageLocation2, boolean isLeaf,
			int rowId_or_pageNo, long no_of_Bytes) {
		// TODO Auto-generated method stub
		try {
			if (isLeaf) {
	...
	private void writePageHeader(int pageLocation, boolean isLeaf,
		...
		try {
			...
			if (isLeaf) {
				...
				pageHeader_Offset_rightPagePointer = binaryFile
						.getFilePointer();
